...
class ngx_recv(angr.SimProcedure):
	def run(self, connection, dst, size):
		dst = overflow_detect.bof_aeg.recv_buf_addr
		fd = 0
		simfd = self.state.posix.get_fd(fd)
		symbolic_size = claripy.BVS('size', 32)
		print(dst)
		#recv = angr.SIM_PROCEDURES['posix']['recv']()
		#realsize = recv(fd, dst, symbolic_size)
		return simfd.read(dst, symbolic_size)
